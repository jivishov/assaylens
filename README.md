...
- `inspect_xtt_workflow` — read workflow state, validation, readiness, concise results, and scientific limitations.
...
2. On the Image screen choose **Load WebMCP demo**. This creates a deterministic synthetic plate image, installs preconfirmed geometry, and leaves the plate map empty so agent edits are visible.
3. Use this prompt:
...
Image loading and geometry confirmation remain human-controlled for real experiments. WebMCP outputs never include raw images, pixels, API keys, local file paths, or complete project files.
...
`webmcp-types` supplies the current ambient TypeScript definitions for `document.modelContext`. In browsers where WebMCP is unavailable, registration is skipped and the normal AssayLens UI continues to function.

## Architecture

See [`docs/WEBMCP.md`](docs/WEBMCP.md) for the bridge architecture, tool contracts, guardrails, and challenge-scope notes.
